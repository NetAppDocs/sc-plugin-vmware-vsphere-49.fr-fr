...
| Autres produits | VSphere Standard, Enterprise ou Enterprise plus
Une licence vSphere est requise pour les opérations de restauration qui utilisent Storage vMotion. Les licences vSphere Essentials ou Essentials plus n'incluent pas Storage vMotion. 


| Destinations principales | Norme SnapCenter : nécessaire pour assurer la protection basée sur les applications sur VMware
SnapRestore : requis pour les opérations de restauration des VM et datastores VMware uniquement
FlexClone : utilisé pour les opérations de montage et de connexion sur les machines virtuelles et les datastores VMware uniquement 


| Destinations secondaires | Norme SnapCenter : utilisée pour les opérations de basculement pour la protection applicative sur VMware
FlexClone : utilisé pour les opérations de montage et de connexion sur les machines virtuelles et les datastores VMware uniquement 
|===
...
| TLS sur le serveur SnapCenter | 1.2, 1.3
Le serveur SnapCenter l'utilise pour communiquer avec le plug-in SnapCenter VMware pour les opérations de protection des données VMDK. 
...
| Port du serveur VMware ESXi | 443 (HTTPS), bidirectionnel
La fonction de restauration de fichiers invité utilise ce port. 


| Plug-in SnapCenter pour port VMware vSphere  a| 
8144 (HTTPS), bidirectionnel
Le port est utilisé pour les communications à partir du client VMware vSphere et du serveur SnapCenter.
8080 bidirectionnel
Ce port est utilisé pour gérer l'appliance virtuelle.
...
| Cluster de stockage ou port de VM de stockage | 443 (HTTPS), bidirectionnel
80 (HTTP), bidirectionnel
Le port est utilisé pour la communication entre l'appliance virtuelle et la machine virtuelle de stockage ou le cluster qui contient la machine virtuelle de stockage. 
|===
...
| Autoriser les utilisateurs à accéder au plug-in SnapCenter pour VMware vSphere | Administrateur de distributeurs sélectifs
...
Ce privilège doit être attribué à la racine du vCenter. 
|===
