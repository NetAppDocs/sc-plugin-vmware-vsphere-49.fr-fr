...
Notez le *moref* pour la machine virtuelle à laquelle vous souhaitez joindre un VMDK. 
...
{backupId}/vmdklocations` | `Get VMDK Locations` Obtient la liste des VMDK pour la machine virtuelle spécifiée.
Passer le *backuPId* de l'étape 2 et le *moref* de l'étape 1.
Notez les informations *vmdkLocationsList*. 


| 4 | `/4.1/vm/{moref}/attachvmdks`  a| 
`Attach VMDKs` Attache le VMDK spécifié à la machine virtuelle d'origine.
Passer le *backuPId* de l'étape 2 et le *moref* de l'étape 1.
Passez la *vmdkLocationsList* de l'étape 3 à l'attribut *vmdklocations*.
...
| 1 | Accédez à `http://<vCenter-IP>/mob` | Recherchez le moref de la VM à partir de l'URL des objets gérés VMware.
Notez le *moref* pour la machine virtuelle sur laquelle vous souhaitez détacher un VMDK. 
...
| 4 | `/4.1/vm/{moref}/detachvmdks` | `Detach VMDKs` Détache le VMDK spécifié.
Passez le *moref* de l'étape 1.
Transmettez les détails VMDK *vmdkLocationsList* de l'étape 3 à l'attribut *vmdksToDetach*. 
|===